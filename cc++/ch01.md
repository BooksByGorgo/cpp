...
In *Gorgo Starting C++* you learned to define classes with constructors, destructors, member functions, and operator overloads.
...
You already know `public` and `private` from *Gorgo Starting C++*.
...
Otherwise only the base destructor runs and the derived part leaks (formally, deleting a derived object through a base pointer without a virtual destructor is undefined behavior):
...
Reusing the `Shape` and `Circle` classes from earlier in this chapter:
...
Downcasting with `dynamic_cast` only works on polymorphic types (types that have at least one virtual function).
You learned `static_cast` in *Gorgo Starting C++* --- `static_cast` does not check at run time and is unsafe for downcasting.
...
The fix is **virtual inheritance** --- the same hierarchy redefined with `virtual` on each direct inheritance from `A`:

```cpp
// alternative version of the same hierarchy
class A2 { public: int value = 42; };
class B2 : virtual public A2 {};
class C2 : virtual public A2 {};
class D2 : public B2, public C2 {};
```

Now `D2` has only one copy of `A2`, shared between `B2` and `C2`.
...
public:
    ...
    std::string play() const override { return "I bet that you look good on the dancefloor"; }
};
...
```
The band is playing:
  Guitar: I bet that you look good on the dancefloor
...
Try giving `Guitar` a data member (say, a `std::vector<std::string>` of tuning notes), then delete the virtual destructor and run with AddressSanitizer (`-fsanitize=address`) to see how it reports the mismatched `delete`.
