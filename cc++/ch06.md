...
`std::string` is versatile, but it owns its data --- copying a long string allocates, and passing a string to a function that only needs to read it can be needlessly expensive.
...
One of its constructors makes that pointer-plus-length nature explicit:
...
`starts_with()` and its counterpart `ends_with()` were added in C++20:

```cpp
bool starts_with(std::string_view sv) const noexcept;  // C++20
bool ends_with(std::string_view sv) const noexcept;    // C++20
```
...
::: {.tip}
**Trap:** The `\w+@\w+\.\w+` pattern above is a *toy* email check, not a real validator.
The actual specification (RFC 5322) allows characters and constructs the pattern rejects --- pluses, dots, hyphens, internationalized addresses, quoted local parts, multi-label domains, and so on --- and rejects strings the pattern accepts.
For real input validation, either send a confirmation email and let the user prove they own the address, or use a dedicated parsing library.
The regex is fine when you just need a smell test; do not block users on it.
:::
...
::: {.tip}
**Tip:** For formatted output, `std::format` (Chapter 10 of *Gorgo Starting C++*) is more flexible than `std::to_string`.
...
|---------|-------------------|------------------------|
| Locale-dependent | Only floating point (`stod`) | No |
