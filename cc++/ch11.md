...
In *Gorgo Starting C++* you learned to read and write file contents with `std::fstream`.
...
::: {.tip}
**Wut:** Notice the surrounding double quotes in every line of output.
The non-member `operator<<` for `fs::path` *quotes* its output to handle paths that contain spaces or special characters --- so `cout << p` prints `"/home/user/music/playlist.m3u"`, not `/home/user/music/playlist.m3u`.
To print without the quotes, call `.string()` (or `.c_str()`):

```cpp
std::cout << p.string() << "\n";    // /home/user/music/playlist.m3u
```

The quoted form is what you want most of the time, but if you are concatenating into a shell command or another file format, reach for `.string()`.
:::
...
::: {.tip}
**Tip:** By default, `recursive_directory_iterator` does not descend into directory symlinks.
Pass `fs::directory_options::follow_directory_symlink` if you want it to follow them.
...
It uses two functions you have not met yet:
...
`temp_directory_path` returns a directory suitable for temporary files (e.g., `/tmp` on Linux).
`relative` returns `p` expressed relative to `base` (like `lexically_relative`, but it resolves symlinks by touching the filesystem).
