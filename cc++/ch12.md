...
Google style uses `CamelCase` for types and functions, and `snake_case` for variables.
...
public:
    ...
    Buffer& operator=(const Buffer& other) {
        if (this != &other) {
            ...
            delete[] data_;                          // ...then release the old buffer
            data_ = new_data;
            size_ = other.size_;
        }
        return *this;
...
Note the order in the copy assignment: allocate the new buffer first, then delete the old one.
If you delete first and `new` throws, `data_` is left dangling and the destructor frees it a second time.
...
## Try It: A CRTP Instance Counter

Here is a program that uses the CRTP to count how many instances of each class are alive.
Because each derived class instantiates its own `Counted<Derived>`, each class gets its own independent counter.
...
class Window : public Counted<Window> {};
class Socket : public Counted<Socket> {};

int main() {
    Window w1, w2;
    std::cout << "Windows alive: " << Window::alive() << "\n";

    {
        Socket s;
        Window w3 = w1;  // copies are counted too
        std::cout << "Windows alive: " << Window::alive() << "\n";
        std::cout << "Sockets alive: " << Socket::alive() << "\n";
    }   // s and w3 are destroyed here
...
Try removing the copy constructor and rerun --- the final `Window` count drops to 1 even though `w1` and `w2` are both still alive.
Can you explain why?
Try adding a third counted class and confirm its counter is independent of the other two.
Try storing some `Window` objects in a `std::vector` and watch the count as the vector grows and is destroyed.
...
- **C++26** will bring `std::execution`, reflection, and contracts; pattern matching is expected in a later standard.
