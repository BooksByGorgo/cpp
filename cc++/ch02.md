...
For a one-argument function this looks like extra ceremony, but it earns its keep when the return type *depends on the parameters*.
...
The trailing form is also required for some lambda return types you will see later, and it is what the deduction guides at the end of this chapter use (`Holder(const char*) -> Holder<std::string>;`).
