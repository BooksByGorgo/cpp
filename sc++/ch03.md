...
Text is everywhere in programs — usernames, messages, file paths, song lyrics.
In C++, the `std::string` type gives you a powerful and easy way to work with text.
Unlike the raw character arrays inherited from C, `std::string` manages its own memory, knows its own length, and provides a rich set of operations for searching, slicing, and combining text.
...
Using `[]` with an invalid index is undefined behavior — anything could happen.
