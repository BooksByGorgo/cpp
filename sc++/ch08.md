...
In the previous chapter you learned how to store data in containers like `std::vector` and `std::array`.
Now you need to do things with that data — sort it, search it, transform it, filter it.
You could write loops for all of this, but C++ provides a rich library of **algorithms** that do the heavy lifting for you.
In this chapter you will learn the most common algorithms, how to write **lambdas** to customize their behavior, and how C++20 **ranges** and **views** make everything cleaner.

## The \<algorithm\> Header

The `<algorithm>` header provides dozens of functions that operate on containers.
Most of them take a pair of iterators (remember `.begin()` and `.end()` from Chapter 7) to define the range of elements to work on.
...
```cpp
std::vector<std::string> songs = {"Creep", "Wonderwall", "Everybody"};
std::sort(songs.begin(), songs.end());
// songs is now {"Creep", "Everybody", "Wonderwall"}
```
...
int main()
{
    std::vector<std::string> playlist = {"Creep", "Wonderwall", "Everybody"};

    auto it = std::find(playlist.begin(), playlist.end(), "Wonderwall");
    if (it != playlist.end()) {
        std::cout << "Found: " << *it << "\n";
    } else {
        std::cout << "Not found\n";
...
int main()
{
    std::vector<std::string> songs = {"Creep", "Wonderwall"};
...
greet("Radiohead");  // Hola, Radiohead!
greet("Oasis");      // Hola, Oasis!
```
...
```cpp
auto it = std::ranges::find(songs, "Wonderwall");
if (it != songs.end()) {
