...
- **Lecture 7 --- Function Basics:** declarations vs definitions, translation units and `static` functions, parameters and return values, pass-by-value, pass-by-reference, `const` parameters, structures as parameters, default parameters
- **Lecture 8 --- Advanced Functions:** `extern` and shared globals, function overloading, recursion, function pointers and callbacks, `[[nodiscard]]`, operator functions
...
- Explain what a translation unit is and use `static` to keep a helper private to one file
...
- Review multiple choice (from lecture 6): **What does this print?**
    ...
    *Instructor note: under the course flags, this snippet also emits two `-Wimplicit-fallthrough` warnings --- expect them in the live demo and point them out.*
...
- Each function has a signature, a body, and at most one return value
...
## 3. Translation Units and `static` Functions (5 min)

- Each `.cpp` file, after its `#include`s are expanded, is a **translation unit**
- The compiler compiles each translation unit independently; the linker combines them into one program
- Suppose two `.cpp` files each want their own private `clamp` helper:
...
- Without `static`, the linker sees two definitions of `clamp` and reports a duplicate definition error --- demo it live
- `static` gives a function **internal linkage**: it is visible only inside its translation unit and never exported to the linker
- It is the C++ way of saying "private to this file"
- An **anonymous namespace** (`namespace { ... }`) does the same job and also works for variables and types

## 4. Parameters and Return Values (5 min)
...
*Instructor note: with `-Wextra`, `try_to_change` emits a `-Wunused-but-set-parameter` warning --- expected, since the copy is written but never read.*

## 6. Pass-by-Reference (12 min)
...
C. You cannot mix `int` and `std::string` parameters
...
- **Read:** chapter 6, remaining sections --- `extern` declarations and shared globals, rvalue references and `std::move`, function overloading, recursion, function pointers, operator functions, `[[nodiscard]]`
- Lectures 18-19 cover move semantics in depth; read the rvalue references section now for the vocabulary
- **Do:** chapter 6 exercises 3-5, 7-10, 12-15
...
- `static` gives internal linkage --- file-local helpers stay private to their translation unit
...
- Share a global variable across translation units with `extern`
...
## 1. `extern` and Shared Globals (5 min)

- Last lecture: `static` makes a helper private to its translation unit
- `extern` is the mirror image: "this name is *defined* in another translation unit"
- Function declarations are implicitly `extern` --- a plain forward declaration already does the job
- Where `extern` earns its keep: sharing a **global variable** across `.cpp` files

```cpp
// globals.h
extern int master_volume;   // declaration only --- no storage
```

```cpp
// audio.cpp
#include "globals.h"
int master_volume = 5;      // the one and only definition
```

```cpp
// video.cpp
#include "globals.h"
// reads and writes the same master_volume
```

::: {.tip}
**Trap:** Writing `int master_volume;` in a second `.cpp` file does not refer to the first one --- it defines a brand-new variable, and the linker reports a duplicate definition.
Put the `extern` declaration in a header and the real definition (with its initializer) in exactly one `.cpp` file.
:::

## 2. Function Overloading (10 min)
...
- Since C++20 you can add a reason that appears in the warning:
...
::: {.tip}
**Trap:** Do **not** overload `&&`, `||`, or `,`. The built-in `&&` and `||` short-circuit; overloaded versions always evaluate **both** sides.
:::
...
## 8. Assignment / Reading (1 min)

- **Read:** chapter 7 of *Gorgo Starting C++*, sections on bases, integer literals, literal suffixes, printing in other bases, and string-to-number conversions (first half)
- **Do:** chapter 7 exercises 1, 2, 7, 13, 17, 18 (base conversions, literals, octal trap, `std::stoi`, suffixes)
- **Bring:** a guess at how `std::stoi("42abc")` behaves

## Key Points to Reinforce

- Share a global across files with an `extern` declaration in a header and one definition in one `.cpp`
