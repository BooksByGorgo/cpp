...
In Chapter 9, you learned how to read and write data using streams.
...
Mixing text and values with lots of `<<` operators quickly gets hard to read.
...
To print a literal brace, double it: `std::format("{{}}")` produces `{}`.
...
You can control how values are formatted by adding specifiers inside the braces, after a `:`.
The grammar of the spec, in the order the pieces must appear, is:

```
[[fill]align][sign][#][0][width][.prec][type]
```

Every piece is optional, but if you include more than one they have to follow this order.
The pieces are introduced one by one in the rest of the section.
...
| `0`      | pad numeric values with zeros, inserted after any sign and base prefix.    |
| `width`  | minimum field width.                                                       |
| `.prec`  | digits after the `.` for `f` floats, else significant digits; max length for strings. |
| `type`   | `d`, `x`, `o`, `b`, `f`, `s`, `p`, etc.                                    |

\index{format specifier!width}
\index{format specifier!alignment}
...
\index{format specifier!alternate form}
\index{format specifier!zero-padding}
...
\index{format specifier!precision}
...
\index{format specifier!dynamic width}
\index{nested replacement field}

**Dynamic width and precision:**

The width does not have to be a number baked into the format string.
Write a nested `{}` where the number would go, and it is filled in from the argument list --- a **nested replacement field**:

```cpp
int w = 12;
std::format("{:>{}}", "hola", w);    // "        hola"
std::format("{:*^{}}", "hola", w);   // "****hola****"
std::format("{:.{}f}", 3.14159, 3);  // "3.142"
std::format("{0:>{1}}", "hola", w);  // same, indexed
```

The nested `{}` is a replacement field like any other, so the no-mixing rule from earlier in this chapter applies: make every field implicit (`{:>{}}`) or every field indexed (`{0:>{1}}`), never a blend.
Dynamic widths are how you line up columns when the width is only known at runtime --- for example, measured from the longest string in a list.
...
These combine formatting and printing into a single call.

```cpp
void std::print(std::format_string fmt, Args... args);
void std::println(std::format_string fmt, Args... args);
```
...
## Try It: Formatting Starter

Here is a program that exercises the formatting tools from this chapter.
...
- Use `{0}`, `{1}`, etc., to reorder or reuse arguments.
You cannot mix implicit `{}` and indexed `{0}` in the same format string.
...
- A nested `{}` in place of a width or precision takes the value from the argument list (`{:>{}}`), for widths only known at runtime.
