...
Chapters to help beginning Java programmers become beginning C programmers.

## Chapters

- DO NOT MODIFY THE AUTHOR INTRO section
- each chapter has a corresponding chapter in @../c4c++ --- try to keep the non-C++ and and non-Java text in sync
...
0. How to use this book:
    ...
    - chapter layout
1. Introduction:
    - java's syntax was derived from C --- it is more similar than it may seem on the surface
...
2. Variables
    ...
    - structures allow data to be grouped together
        - unlike java, only member variables allowed -- no member functions; although, you can use function pointers (described later)
        - members accessed with . operator
        - assignment does a copy
3. Strings
    - there is no String. everything revolves around arrays of characters that end with a null '\0'
...
6. Pointers
    - pointers may seem like a something foreign to java programmers, but they are everywhere! what do you get when you try to use a null object? a null pointer exception! every object reference is a pointer!
    ...
    - in java whenever use use . with an object, you are actually doing a -> on a pointer to that object
    - pass by value with pointer workaround
       - remember all objects are pointers in Java, that is why
         ```
        ...
        addTo(sb, i);
        ```
        after the call to addTo, sb will be `!!!` and i will still be 0 because a pointer to sb was passed to addTo but i was passed as the value 0.
7. Functions
