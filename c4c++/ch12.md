...
\index{atexit}
`exit` also calls any functions registered with `atexit` and then flushes all open `stdio` streams before terminating the program.
...
// (gethostbyname is obsolete — use getaddrinfo — but it is a classic example)
```
