...
`exit` runs the registered functions first and then flushes all open `stdio` streams before terminating the program:
...
Reverse order is deliberate: it unwinds resources opposite to the order you set them up, the same way C++ destructors run.
...
// (gethostbyname is obsolete — use getaddrinfo — but it's a classic)
```
...
\index{strerror}
Many C library functions set the special variable `errno` (declared in `<errno.h>`, and thread-local on modern systems) to indicate what went wrong.
...
Only one member can hold a value at a time, and the size of the union is at least the size of its largest member (padding can round it up):
...
Notice the double cast in `compare_strings`: `qsort` passes a pointer *to* each array element, and each element is already a `const char *`, so you receive a `const char **` disguised as `const void *`.
...
- `exit` calls `atexit` handlers and then flushes `stdio` streams before terminating.
