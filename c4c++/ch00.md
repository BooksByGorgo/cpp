...
When this booklet introduces a new function, it shows the function's **signature** (its prototype).
A function's signature gives you three things at a glance: what the function gives back (return type), what it is named, and what goes in (parameter list).
For example:
...
This tells you that `strlen` takes a single `const char *` parameter `s` and returns a `size_t`.
Reading function prototypes is essential when working with C libraries and header files, so getting comfortable with them early pays off.
...
Near the end of each chapter is a starter program that you can type in and experiment with.
