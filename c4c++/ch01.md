...
We will use modern C (C11 or later) throughout this book.

My go-to C textbook is *The C Programming Language (Second Edition)* by Brian Kernighan and Dennis Ritchie --- often called "K&R" [@KernighanRitchie1988].
...
| References (`&`) | Pointers (`*`) |
| `bool` (built-in) | `_Bool` or `<stdbool.h>`; keyword since C23 |
...
```c
double score = 98.6;
printf("Score: %f\n", score);    // 98.600000  (6 decimals by default)
...
C doesn't have references the way C++ does.
Instead, we give `scanf` the **address** (location in memory) of each variable, not the value.
You obtain the address of a variable using the `&` operator.
(It's kind of confusing that `&` is also used for references, right?)
...
The compiler will warn you that it is wrong, but it will still compile the code.
You can try to run it, but it will not work.
:::

Strings in C are an array of characters.
A variable representing an array takes on the address of the first element of the array when used in expressions or passed to functions.
