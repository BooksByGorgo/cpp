...
As you saw in Chapter 1, `scanf` needs the *address* of the variable so it can store the result there.
...
Instead of passing a `char[]`, you pass the address of a `char *` variable (`char **`), and `scanf` allocates exactly enough memory and puts the address of the allocated memory into the variable:
...
::: {.tip}
**Tip:** `%m` is a POSIX extension (available with glibc on Linux) and is not part of the C standard.
It will not work with MSVC on Windows, and macOS and the BSDs do not implement it either.
Because it is non-standard, `gcc -Wall` warns about it ("ISO C does not support the 'm' scanf flag").
...
On Unix systems, binary and text modes behave identically.
...
\index{snprintf}
`sprintf` has the same buffer overflow risk as `strcpy` --- it does not check the size of the destination buffer.
...
```c
char *msg;
asprintf(&msg, "Track %02d: %s", 3, "La Bamba");
printf("%s\n", msg);    // "Track 03: La Bamba"
...
On success it always null-terminates the result (on end of file with nothing read, it returns `NULL` and leaves the buffer alone):
...
`stderr` is unbuffered by default, which is why error messages appear immediately.
:::
