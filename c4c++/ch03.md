...
In C, there is no string type at all.
\index{null terminator}
A "string" in C is just an array of `char` that ends with a null character `'\0'`.
...
```c
char arr[] = "I Can't Drive 55";      /* modifiable stack copy */
const char *ptr = "I Can't Drive 55"; /* points to read-only memory */
```
...
bad[0] = 'i';                   /* UNDEFINED BEHAVIOR: likely crash */
```
...
if (a == b) {
    // This compares ADDRESSES, not content — always false (gcc warns)
}
...
It is available on virtually every system you will use, but under `-std=c99` or `-std=c11` the declaration is hidden and modern compilers reject the call as an implicit function declaration.
:::
...
`strtok_r` takes an extra `char **saveptr` parameter to store the tokenizer state, making it safe to use in multi-threaded programs and allowing nested tokenization (Microsoft's `strtok_s` works the same way; the C11 Annex K version adds a buffer-size parameter too):
...
The buffer is 12 bytes, `"Buenas "` uses 8 (7 characters plus its `'\0'`), and `strcat` overwrites that `'\0'` and appends `"noches"` plus a new `'\0'` --- 6 more bytes beyond the original 8.
...
- `strcpy` and `strcat` do not check buffer bounds.
  Prefer `strncpy` and `strncat`.
- `strdup` allocates memory with `malloc` --- you must `free` the result.
- `strtok` splits strings into tokens but modifies the original string and is not thread-safe.
  Use `strtok_r` or `strtok_s` instead.
- `<ctype.h>` provides character classification (`isalpha`, `isdigit`, etc.) and conversion (`toupper`, `tolower`).
