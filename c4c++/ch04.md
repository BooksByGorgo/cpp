...
And C uses `int` for boolean results --- there is no built-in `bool` type (though C99 added `_Bool` and `<stdbool.h>`, and C23 finally made `bool` a keyword).
...
C99 added `_Bool` as a keyword and `<stdbool.h>` as a convenience header that defines `bool`, `true`, and `false` (in C23 all three finally became keywords, matching C++):
