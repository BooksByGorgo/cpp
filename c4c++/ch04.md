...
And C uses `int` for boolean results --- you get `bool` from `<stdbool.h>` (C23 finally made it a keyword).
...
The `_Bool` keyword and the `<stdbool.h>` convenience header give you `bool`, `true`, and `false` (in C23 all three finally became keywords, matching C++):
...
- C uses `int` for boolean results: `0` is false, nonzero is true.
  Include `<stdbool.h>` for `bool`, `true`, and `false` (keywords in C23).
