...
There are no classes, no `std::string`, and (until C23) no `bool` keyword without a header.
...
Without it (before C23), you must use `_Bool` for the type and integer values `0` and `1`.
C23 finally made `bool`, `true`, and `false` keywords, so the newest compilers do not need the header at all.
...
For any type whose size is known at compile time, it evaluates at compile time --- there is no runtime cost; the one exception is explained in the Memory chapter.
...
```c
/* OK — column size specified */
void print_grid(int grid[][4], int rows);
```
...
PI = 3.1;    // ERROR: cannot change a const
```
...
// prints: I Love Rock 'n' Roll (1981)
...
The `->` operator accesses a member through a pointer --- `s->title` is shorthand for `(*s).title`.
...
```c
struct song track = {.year = 1984, .title = "I Can't Drive 55"};
```
