...
Treat `atoi` as something you read in other people's code, not something you write.
:::
...
When you use a value of an integer type smaller than `int` (like `char` or `short`) in an expression, it is automatically promoted to `int` if `int` can hold all the values of the type.
...
::: {.tip}
\index{size\_t}
**Trap:** A related surprise comes from unsigned arithmetic with `strlen`.
